...
export default class BotCommand extends Command {
    ...
    async execute(client: SoapClient, interaction: CommandInteraction) {
        ...
        const [guilds, economy_wallet, economy_bank, economy_max_bank, users, items, commands] = await Promise.all([
        ...
        const remaining = getTimeRemaining(`${client.readyAt?.toISOString()}`, `${(new Date(Date.now()).toISOString())}`)
        ...
        const StatsEmbed = new MessageEmbed()
            .setColor("#ff00e4")
            .setAuthor({ name: "Soap BOT stats", iconURL: "https://skippies.fun/discord/SoapBOT/images/soap.png" })
            .addFields([
                { name: `\u200B`, value: `**Servers**: ${guilds}` },
                { name: `\u200B`, value: `**Users**: ${users[0].countusers.toLocaleString()}` },
                { name: `\u200B`, value: `**Commands**: ${commands[0].countcommands.toLocaleString()}` },
                { name: `\u200B`, value: `**Items**: ${items[0].countitems.toLocaleString()}` },
                { name: `\u200B`, value: `**Total soap in hands**: 🧼${economy_wallet[0].sumpoints.toLocaleString()}` },
                { name: `\u200B`, value: `**Total soap in stashes**: 🧼${economy_bank[0].sumpoints.toLocaleString()}` },
                { name: `\u200B`, value: `**Total soap that can be stored in stash**: 🧼${economy_max_bank[0].sumpoints.toLocaleString()}` },
                { name: `\u200B`, value: `**Total soap in hands and stashes combined**: 🧼${(economy_wallet[0].sumpoints + economy_bank[0].sumpoints).toLocaleString()}` },
