...
export default class BotCommand extends Command {
    ...
    async execute(client: SoapClient, interaction: CommandInteraction) {
        ...
        if (!victim) {
            interaction.reply({ content: `**${mention.displayName}** doen't even have 🧼**${min_threshhold.toLocaleString()}**. Not worth` });
            return false
        }
        ...
        if (robber.points < min_threshhold) {
            interaction.reply({ content: `You need atleast 🧼**${min_threshhold.toLocaleString()}** to rob someone. Imagine being so poor lmao` });
            return false
        }
        ...
        if (hardenerCheck) {
            ...
            interaction.reply({ content: `You tried to steal from **${mention.displayName}** but when you tried to lift their soap, you realized it's 69x heavier. You ended up losing **🧼${lose}**.` })
            await dmUser(mention, { embeds: [failDm] })
            await Promise.all([setPoints(mention.id, victim.points + lose), setPoints(user.id, robber.points - lose)]);
        ...
        if (success) {
            ...
            await Promise.all([setPoints(mention.id, victim.points - stolenAmount),
            setPoints(user.id, robber.points + stolenAmount)])
            interaction.reply({ content: `You stole 🧼**${stolenAmount.toLocaleString()}** from **${mention.displayName}**! (${Math.round(percent * 100)}% of their total 🧼).` });
        } else {
            ...
            await Promise.all([setPoints(mention.id, victim.points + lostAmount),
            setPoints(user.id, robber.points - lostAmount)])

            interaction.reply({ content: `You tried to steal from **${mention.displayName}** but **slipped** on your soap and paid 🧼**${lostAmount.toLocaleString()}**` });
        }
