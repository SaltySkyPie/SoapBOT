...
export default class BotCommand extends Command {
    ...
    async execute(client: SoapClient, interaction: CommandInteraction) {
        ...
        const member_points = await SQL("SELECT u.user_id, u.points, @rownum := @rownum + 1 AS rank FROM users u, (SELECT @rownum := 0) r WHERE user_id IN (?) ORDER BY u.points DESC LIMIT 10", [members])
        ...
        for (const position of member_points) {
            ...
            LeaderboardEmbed.addFields({ name: '\u200B', value: `**#${position.rank}** **${nickname}** - 🧼 ${(position.points).toLocaleString()}`, inline: false })
