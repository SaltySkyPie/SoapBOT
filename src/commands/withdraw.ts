...
export default class BotCommand extends Command {
    ...
    async execute(client: SoapClient, interaction: CommandInteraction) {
        ...
        const [[current_stash_balance, max_stash_balance], current_points] = await Promise.all([getBank(user.id), getPoints(user.id)])

        const entered_amount = interaction.options.getString("amount")

        let decoded = (entered_amount == "max" || entered_amount == "all") ? current_stash_balance : await decodeNumber(entered_amount!)
        ...
        interaction.reply({ content: `You withdrew 🧼**${decoded.toLocaleString()}** from your stash. Your current stash balance is 🧼**${(current_stash_balance - decoded).toLocaleString()}** and hand balance 🧼**${(current_points + decoded).toLocaleString()}**.` })
        return true
    }
    ...
    async getSlash(): Promise<SlashCommandBuilder | Omit<SlashCommandBuilder, "addSubcommandGroup" | "addSubcommand">> {
        return new SlashCommandBuilder()
            ...
            .addStringOption(option => option.setName("amount").setDescription("Amount of 🧼 you want to withdraw").setRequired(true));
    }
}
