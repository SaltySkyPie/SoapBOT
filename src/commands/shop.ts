import { CommandInteraction, MessageEmbed, MessageActionRow, MessageButton, Message } from "discord.js";
...
export default class BotCommand extends Command {
    ...
    async execute(client: SoapClient, interaction: CommandInteraction) {
        let currentPage = 0;
        const count = await SQL(`SELECT count(id) as count FROM items WHERE shop=1 AND stock!=0`);
        const maxPage = Math.ceil(count[0].count / 5) - 1;
        const getItems = async (page = 0) => {
            const all = await SQL(`SELECT * FROM items WHERE shop=1 AND stock!=0 LIMIT ${page * 5}, 5`)
            return all
        }
        ...
        const reply = await interaction.reply({ embeds: [ShopEmbed], components: [row], fetchReply: true }) as Message;
