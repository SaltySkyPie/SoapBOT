...
export default class BotCommand extends Command {
    ...
    async execute(client: SoapClient, interaction: CommandInteraction) {
        ...
        const getItems = async (page = 0) => {
            const all = await SQL(`SELECT * FROM commands ORDER BY command LIMIT ${page * 5}, 5`)
            return all
        }
        ...
        const reply = await interaction.reply({ embeds: [HelpEmbed], components: [row], fetchReply: true }) as Message
        ...
        collector.on('collect', async (i) => {
            ...
            if (i.user.id === interaction.user.id) {
                ...
                items.forEach((i: any) => {
                    HelpPageEmbed.addFields({ name: '\u200B', value: `**/${i.command}**\n`, inline: true }, { name: '\u200B', value: `${i.description}\n`, inline: true }, { name: '\u200B', value: `-\n`, inline: true })
                });
