...
export default class BotCommand extends Command {
    ...
    async execute(client: SoapClient, interaction: CommandInteraction) {
        ...
        const [side, bet] = [interaction.options.getString('side'), interaction.options.getString('bet')]
        ...
        let decoded = (bet == "max" || bet == "all") ? points : await decodeNumber(bet!)
        ...
        const flipping = new MessageEmbed()
            ...
            .setAuthor({ name: `${(interaction.member as GuildMember).displayName}'s coin flip`, iconURL: avatar })
            .setDescription(`is betting 🧼**${decoded.toLocaleString()}** on ${side}.`)
            .setColor("#ff00e4")


        const reply = (await interaction.reply({ embeds: [flipping], fetchReply: true })) as Message

        const result_image = success ? "https://skippies.fun/discord/soapbot/gifs/flip-success.gif" : (side.toLowerCase() == "heads" ? "https://skippies.fun/discord/soapbot/gifs/heads-fail.gif" : "https://skippies.fun/discord/soapbot/gifs/tails-fail.gif")

        const result = new MessageEmbed()
            .setColor("#ff00e4")
            .setTitle(`${success ? `**${(interaction.member as GuildMember).displayName}** won the coin flip!` : `**${(interaction.member as GuildMember).displayName}** lost the coin flip!`}`)
            .setAuthor({ name: `${(interaction.member as GuildMember).displayName}'s coin flip`, iconURL: avatar })
    ...
    async getSlash(): Promise<SlashCommandBuilder | Omit<SlashCommandBuilder, "addSubcommandGroup" | "addSubcommand">> {
        return new SlashCommandBuilder()
            ...
            .addStringOption(option => option.setName('side').setDescription('Side you want to bet on').setRequired(true)
                .addChoice('Heads', 'heads')
                .addChoice('Tails', 'tails'))
            .addStringOption(option => option.setName('bet').setDescription('How much soap do you bet?').setRequired(true));
    }
}
