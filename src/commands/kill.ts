...
export default class BotCommand extends Command {
    ...
    async execute(client: SoapClient, interaction: CommandInteraction) {
        ...
        const kill = ((await SQL("SELECT message FROM kill_msg ORDER BY RAND() LIMIT 1", []))[0].message).toString().replaceAll('{1}', `**${user.displayName}**`).replaceAll('{0}', `**${target.displayName}**`)
