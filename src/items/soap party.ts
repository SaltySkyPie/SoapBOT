import { CommandInteraction, GuildMember, MessageEmbed, MessageActionRow, MessageButton, Message } from "discord.js";
...
export default class BotItem extends Item {
    ...
    async execute(client: SoapClient, interaction: CommandInteraction) {
        ...
        const image = (await SQL("SELECT link FROM gifs WHERE purpose=2 ORDER BY RAND() LIMIT 1", []))[0].link
        ...
        const reply = await interaction.reply({ embeds: [PartyEmbed], components: [row], fetchReply: true }) as Message
        ...
        collector.on('collect', async (i) => {
            ...
            interaction.channel!.send(`**${(i.member as GuildMember).displayName}** joined the **SOAP PARTY** and obtained **🧼${earned.toLocaleString()}**!`)
            joined++
            i.deferUpdate().catch()
        })
        ...
        collector.on('end', (collected) => {
            ...
            interaction.channel!.send(`**${user.displayName}**'s party ended with total of **${joined.toLocaleString()}** participants!`)
        })
